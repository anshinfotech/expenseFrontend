...
const CustomLineChart = ({ expenses }) => {
  ...
  const processExpensesData = () => {
    ...
    const expensesByDate = expenses.reduce((acc, expense) => {
      const date = moment(expense.paymentDate).format('YYYY-MM-DD');
      if (!acc[date]) {
        acc[date] = {
          total: 0,
          transactions: 0,
      ...
      acc[date].total += expense.expense;
      acc[date].transactions += 1;
      acc[date].byType[expense.paymentType] = (acc[date].byType[expense.paymentType] || 0) + expense.expense;
      return acc;
    }, {});
    ...
    return allDays.map(day => {
      ...
      return {
        ...
        salary: dayData?.byType['Salary'] || 0,
        rent: dayData?.byType['Rent'] || 0,
        miscellaneous: dayData?.byType['Miscellaneous'] || 0,
      };
    });
  };
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            <defs>
              ...
              <linearGradient id="miscGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="#65A30D" stopOpacity={0.8} />
                <stop offset="95%" stopColor="#65A30D" stopOpacity={0.1} />
              </linearGradient>
            </defs>
